...
Page({
	...
	async getGoodsList() {
		...
		//   // console.log("item.minPrice",item.minPrice)
		// 	// console.log("item.minPrice*wx.getStorageSync('rebate') ",item.minPrice*wx.getStorageSync('rebate'));
		// 	item.minPrice = (item.minPrice*wx.getStorageSync('rebate')).toFixed(2)
		// })
		// console.log(list)
		this.setData({
	...
	async addShopCarDone(options) {
		...
		if (res.code == 30002) {
			// 需要选择规格尺寸
			const skuCurGoodsRes = await WXAPI.goodsDetail(options.goodsId,wx.getStorageSync('token'))
