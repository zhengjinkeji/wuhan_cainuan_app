...
Page({
	...
	async getGoodsDetailAndKanjieInfo(goodsId) {
		const that = this;
		const goodsDetailRes = await WXAPI.goodsDetail(goodsId,wx.getStorageSync('token'))
    ...
    if(this.data.buyNumMin>num){
      this.setData({
        buyNumber: this.data.buyNumMin
      })
    }else if(this.data.buyNumMax<num){
      this.setData({
	...
	async labelItemTap(e) {
		...
		// 计算当前价格
		if (canSubmit) {
			const res = await WXAPI.goodsPrice(this.data.goodsDetail.basicInfo.id, propertyChildIds,wx.getStorageSync('token')) 
			if (res.code == 0) {
